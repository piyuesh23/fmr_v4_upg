...
/*-- custom code for highwire file backup when copying between two different repositories. --*/
  ...
  function cross_create_backup($dest, $files, $action, $dest_repo_id) {
    $count = count($files);
    for($i=0; $i<$count; $i++) {
      $file = $files[$i];
      $filetocopy = $this->urlBase.$file;
      error_log(print_r($this->repository, TRUE));
      ...
      if(strpos($filetocopy, 'backup') === false) {

        $date = $this->getTime();
        $selected_dir = substr($file, 0, strrpos($file, '/'));
        $filename_orig = substr($file, strrpos($file, '/')+1);
        if(strpos($filename_orig, '.')) {
          $fname_without_extension = substr($filename_orig, 0, strrpos($filename_orig, '.'));

          $extension = substr($filename_orig, strrpos($filename_orig, '.'));
        }
        else {
          $fname_without_extension = $filename_orig;
        }

        $fname = $fname_without_extension.'_'.$date['year'].$date['mon'].$date['mday'].'-'.$date['hours'].$date['minutes'].$date['seconds'].$extension;

        $dest_dir = 'ajxp.fs://'.$dest_repo_id.$dest.'/backup/';

        if(!is_dir($dest_dir)) {
          mkdir($dest_dir, 0777);
        }

        $dest_file = 'ajxp.fs://'.$dest_repo_id.$dest.'/backup/'.$fname;

        if($dest != '/recycle_bin') {
          $files_ext = $this->getFiles('ajxp.fs://'.$dest_repo_id.$dest);
          $selectedfile = substr($file, strrpos($file, '/')+1);
          if($files_ext != null) {
            if(in_array($selectedfile, $files_ext)) {
              if(!is_dir($dest_dir)){
                mkdir($dest_dir);
              }
              error_log($this->urlBase.$dest.'/'.$selectedfile);
              error_log($dest_file);
              copy('ajxp.fs://'.$dest_repo_id.$dest.'/'.$selectedfile , $dest_file);
            }
          }
        }
      }

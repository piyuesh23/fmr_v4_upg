...
Class.create("UserWidget", {
	...
	updateGui : function(){
		...
		if(oUser != null)
		{
			if(oUser.id != 'guest')
			{
        var uid_uniquified = oUser.id;
        var href = document.location.href;
        var ac_url = href.substring(href.indexOf('ac-url'));
        var ac_url = ac_url.substring(ac_url.indexOf('/')+2, ac_url.lastIndexOf('/'));
        var ac_name = ac_url.substring(0,ac_url.indexOf('.'));

        if(uid_uniquified != 'admin')
          var uid_hw = uid_uniquified.substring(0, uid_uniquified.lastIndexOf('_'));
//          var uid_hw = uid_uniquified.substring(0,uid_uniquified.indexOf('_'+ac_name));
        else
          var uid_hw = uid_uniquified;
//				logging_string = '<div class="user_widget_label"><ajxp:message ajxp_message_id="142">'+MessageHash[142]+'</ajxp:message><i ajxp_message_title_id="189" title="'+MessageHash[189]+'">'+ uid_hw+' </i></div><div class="inlineBarButtonLeft" style="-moz-border-radius: 0pt 5px 5px 0pt;border-radius: 0pt 5px 5px 0pt;border-left-style:none; border-width:1px;"><img width="16" height="16" style="height: 6px; width: 10px; margin-top: 9px; margin-left: 3px; margin-right: 3px;" ajxp_message_title="189" title="'+MessageHash[189]+'" src="'+ajxpResourcesFolder+'/images/arrow_down.png"></div>';

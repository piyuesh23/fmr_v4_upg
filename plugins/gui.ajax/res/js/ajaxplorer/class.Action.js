...
Class.create("Action", {
	...
	fireContextChange: function(){
		...
		if((this.options.name == "publish")||(this.options.name == "revisions")) {
		  if(ajaxplorer.user){
	      if(ajaxplorer.user.activeRepository == "ajxp_conf") {
		      this.hideForContext();
		    }
		  }
		}
		var rightsContext = this.rightsContext;
		if(this.options.name == "copy") rightsContext.write = true;
		...
		if((this.context.behaviour == "hidden")||(this.context.dir == "hidden")) {
		  return this.hide();
		}
...
//    if((this.options.name == "open_with")||(this.options.name == "download")||(this.options.name == "slideshow_sel")) {
//      //do nothing-- check applied for displaying buttons in read-only repo on IE.
//    }
//    else{
...
//		}
		...
		if(this.options.name == "copy"){
		  selectionContext.file = true;
		  selectionContext.dir = true;
		}
//    if(userSelection != null) {
//      var file = userSelection._selectedNodes[0]._path;
//    }
//    if((selectionContext.symlink == false)&&(typeof file != undefined)) {
//      var conn = new Connexion();
//      conn.addParameter('get_action', 'check_symlink');
//      conn.addParameter('file', file);
//      conn.onComplete = function(transp){
//        if(transp.responseText == "false") {
//          ajaxplorer.resp_symlink = "false";
//        }
//        else ajaxplorer.resp_symlink = "true";
//      };
//      conn.sendSync();
//      if(ajaxplorer.resp_symlink == "false") {
//        ajaxplorer.resp_symlink = "true" ;
//        return this.hide();
//      }
//    }
	  ...
	  if(selectionContext.allowedMimes.size()){
		...
		if((this.options.name == "publish")||(this.options.name == "revisions")) {
		  if(userSelection != null) {
  		  if(userSelection._rootNode._label == "Settings") {
	  	    this.hide();
	  	  }
	  	}
		}

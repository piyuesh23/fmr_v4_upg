...
Class.create("FilesList", SelectableElements, {
	...
	fill: function(contextNode){
		...
		for (var i = 0; i < children.length ; i++)
		{
			...
			var newItem;
			var metadata_obj = child._metadata._object;

		  if(metadata_obj.symlink == "true") {
  			if(this._displayMode == "list") {
	...
	ajxpNodeToDiv: function(ajxpNode){
		...
		if(metadata._object.symlink == "true") {
      label.addClassName('symlink');
		}

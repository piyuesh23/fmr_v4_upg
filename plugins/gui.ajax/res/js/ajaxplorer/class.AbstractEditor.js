...
Class.create("AbstractEditor" , {
	...
	initialize : function(oContainer, options){
		...
		this.initActions();
		modal.setCloseAction(function(){this.getRepoId();this.close();}.bind(this));
	},
	...
	setModified : function(isModified){
        ...
        if(this.canWrite == false)
          this.actions.get("saveButton").addClassName("disabled");
				this.actions.get("saveButton").show();
			}else{
	 ...
	 */
	close : function(){
    ajaxplorer.ckeditor_close = true;
    if(typeof CKEDITOR != 'undefined') {
      if(typeof CKEDITOR.instances.content != 'undefined') {
        this.repoid = CKEDITOR.instances.content.config.repoId;
      }
    }
		if(this.fullScreenMode){
			this.exitFullScreen();
    ...
    if((this.repoid != undefined)&&(this.repoid != ajaxplorer.repoid)) {
      ajaxplorer.triggerRepositoryChange(this.repoid);
    }
...
/**
 * Custom function to get current repoid --required in case
 * of ckeditor to handle adding images from different repo.
 */
  getRepoId : function() {
    var connexion = new Connexion();
		connexion.addParameter('get_action', 'get_repoid');

		connexion.onComplete = function(transp){
      ajaxplorer.repoid = transp.responseText;
		}.bind(this);
		connexion.setMethod('get');
    connexion.sendSync();
  }
